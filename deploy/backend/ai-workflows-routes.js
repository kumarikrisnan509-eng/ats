...
// T-248: `mfData` arg removed -- /mf-pick workflow that consumed it is retired.
...
Return JSON verdict only.`;
  ...
  // T-248: /mf-pick AI workflow RETIRED. The original ~140-line handler burned
  // LLM tokens to recommend mutual funds the platform could never actually
  // place orders for -- Kite Connect's MF API is GET-only by Zerodha/SEBI
  // bank-mandate design. The workflow was a misleading affordance.
  //
  // For long-term passive investing, see #longterm with exchange-traded ETF
  // baskets that ARE fully buyable through /api/orders/place. git blame
  // e487a980 has the original handler if a forensic look is ever needed.
  //
  // Compat: 410 Gone stub for ~30 days. Drop on or after 2026-06-19.
  router.post('/mf-pick', (_req, res) => res.status(410).json({
    ok: false, reason: 'gone',
    detail: 'AI mf-pick retired in T-248. Kite Connect MF API is read-only by Zerodha/SEBI design; platform never had MF placement. Long-term investing moved to ETF baskets at #longterm.',
  }));
