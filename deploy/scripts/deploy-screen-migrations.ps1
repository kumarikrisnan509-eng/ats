...
Write-Host "  Deploy screen migrations: dashboard, trading, tax → /api/*" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    & git add `
        ...
        src/screen-trading.jsx `
        src/screen-tax.jsx `
        2>&1 | ForEach-Object { Write-Host "    $_" -ForegroundColor DarkGray }
    & git commit -m "screens: migrate dashboard/trading/tax off inline hardcoded data; gate demo via MockData.isDemoOn" 2>&1 | ForEach-Object { Write-Host "    $_" -ForegroundColor DarkGray }
...
Write-Host ""
Write-Host "[2/4] Wait for CI + deploy (5-7 min)" -ForegroundColor Yellow
$start   = Get-Date
...
echo
echo "  /api/health (truncated)"
curl -sS --max-time 5 http://127.0.0.1:8080/api/health | head -c 400
echo
echo
echo "  /api/orders"
curl -sS --max-time 5 http://127.0.0.1:8080/api/orders | head -c 400
echo
echo
echo "  /api/portfolio/positions"
curl -sS --max-time 5 http://127.0.0.1:8080/api/portfolio/positions | head -c 400
echo
'@
$bashScript = $bashScript.Replace('__NEW_TAG__', $newTag).Replace('__OWNER__', $RepoOwner).Replace('__PAT__', $GhcrPat)
$OutFile = Join-Path $ProjectRoot "deploy\.deploy-screen-migrations-output.txt"
...
try {
    ...
    Write-Host "    health: ok=$($health.ok) broker.connected=$($health.broker.connected) subscribedInstruments=$($health.broker.subscribedInstruments) instruments.size=$($health.broker.instruments.size)" -ForegroundColor Green
